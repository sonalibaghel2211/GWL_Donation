...
export default function PosDonation() {
    ...
    const { settings: savedSettings, currency: currencyCode, logs, analytics, plan, subscription, blockConfig, shop } = loaderData;
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                            <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
                                <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: hasActiveSubscription(subscription, "canUsePercentageDonation") ? "pointer" : "not-allowed", fontSize: "14px", opacity: hasActiveSubscription(subscription, "canUsePercentageDonation") ? 1 : 0.6 }}>
                                    <input
                                        type="radio"
                                        disabled={!hasActiveSubscription(subscription, "canUsePercentageDonation")}
                                        checked={settings.donationType === "percentage"}
                                        onChange={() => handleTypeSwitch("percentage")}
                                        style={{ accentColor: "#6C4A79" }}
                                    />
                                    Percentage (%)
                                    {!hasActiveSubscription(subscription, "canUsePercentageDonation") && <s-badge tone="caution">Advanced</s-badge>}
                                </label>
                            ...
                            {!hasActiveSubscription(subscription, "canUsePercentageDonation") && (
                                <s-banner tone="info">
            ...
            blocks={[
                {
                    ...
                    instructions: [
                        ...
                        " ➺ Select ", "Portion of Sale Donation"
                    ],
                },
                {
                    id: "pos-cart",
