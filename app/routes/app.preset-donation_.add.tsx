...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const [shopResponse, planSubscription, campaignCount] = await Promise.all([
    admin.graphql(`
      query {
          shop {
              currencyCode
  ...
  const canCreateCampaign = maxCampaigns === null || campaignCount < maxCampaigns;
  const canUseRecurringDonations = planFeatures.canUseRecurringDonations;

  return { currency, canCreateCampaign, canUseRecurringDonations, currentPlan };
};
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const planSubscription = await prisma.planSubscription.findUnique({ where: { shop } });
    const currentPlan = (planSubscription?.plan || "basic") as PlanType;
    const planFeatures = PLAN_FEATURES[currentPlan] || PLAN_FEATURES.basic;
    ...
    if (maxCampaigns !== null) {
      const existingCount = await prisma.campaign.count({ where: { shop } });
      if (existingCount >= maxCampaigns) {
        return data(
          { success: false, error: `Campaign limit reached for your ${currentPlan.charAt(0).toUpperCase() + currentPlan.slice(1)} plan. Please upgrade to create more campaigns.` },
          { status: 403 },
        );
      }
    }
    ...
    // Force-disable recurring if plan doesn't allow it
    const isRecurringEnabled = planFeatures.canUseRecurringDonations
      ? formData.get("isRecurringEnabled") === "true"
      : false;
...
export default function AddCampaignPage() {
  const { currency, canCreateCampaign, canUseRecurringDonations, currentPlan } = useLoaderData<typeof loader>();
    ...
    <s-page heading="Add Campaign">
      ...
      {!canCreateCampaign ? (
        <div style={{ padding: "40px 20px", textAlign: "center" }}>
          ...
          <h2 style={{ fontSize: "20px", fontWeight: 700, color: "#1a1c1d", marginBottom: "8px" }}>Campaign Limit Reached</h2>
          <p style={{ fontSize: "14px", color: "#6d7175", marginBottom: "24px", maxWidth: "400px", margin: "0 auto 24px" }}>
            Your <strong>{currentPlan.charAt(0).toUpperCase() + currentPlan.slice(1)}</strong> plan allows only 1 campaign. Upgrade to unlock unlimited campaigns.
          </p>
          <button
            onClick={() => navigate("/app/pricing")}
            style={{ padding: "12px 28px", background: "#6C4A79", color: "#fff", border: "none", borderRadius: "10px", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}
          >
            Upgrade Plan
          </button>
        </div>
