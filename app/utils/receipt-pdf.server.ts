...
// ─── Helper: strip HTML tags & CSS from template ───────────
function stripHtml(html: string): string {
    return html
        // First, completely remove <style> blocks and their content
        ...
        .replace(/<\/?(p|div|span|strong|em|b|i|u|a|h[1-6]|ul|ol|li|table|tr|td|th|thead|tbody|img|hr|blockquote|html|body|meta|link|script|!DOCTYPE|!--)[^>]*>/gi, "")
...
export async function generateReceiptPDF(args: ReceiptPDFArgs): Promise<Buffer> {
    ...
    // Check custom template header title / subtitle
    const rawTemplate = isVoided
        ? (args.cancelAcknowledgementText || "")
        : (args.acknowledgementText || "");

    let headerTitle = isVoided ? "CANCELLATION RECEIPT" : "DONATION RECEIPT";
    if (rawTemplate) {
        const h1Match = rawTemplate.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i);
        if (h1Match && h1Match[1]) {
            const extracted = stripHtml(h1Match[1]).trim();
            if (extracted) headerTitle = extracted;
        }
    }

    let headerSubtitle = "Smart Donate · Recurring & Receipts";
    if (rawTemplate) {
        const subMatch = rawTemplate.match(/class=["']header-title["'][^>]*>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>/i);
        if (subMatch && subMatch[1]) {
            const extracted = stripHtml(subMatch[1]).trim();
            if (extracted) headerSubtitle = extracted;
        }
    }

    doc.font("Helvetica-Bold").fontSize(16).fillColor(COLORS.white)
        .text(headerTitle, headerTextX, headerTextY);
    doc.font("Helvetica").fontSize(9).fillColor(COLORS.white).fillOpacity(0.85)
        .text(headerSubtitle, headerTextX, headerTextY + 20);
    ...
    if (isVoided) {
        ...
        let voidText = "VOIDED — This donation has been cancelled. This receipt is not valid for tax deduction purposes.";
        if (rawTemplate) {
            const voidMatch = rawTemplate.match(/class=["']void-banner["'][^>]*>([\s\S]*?)<\/div>/i);
            if (voidMatch && voidMatch[1]) {
                const extracted = stripHtml(voidMatch[1]).trim();
                if (extracted) voidText = extracted;
            }
        }
    ...
    let footerLine1 = "This receipt was generated by Smart Donate · Recurring & Receipts. Please retain this document for your records.";
    let footerLine2 = pdfArgs.footerNote || `Generated on ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`;
    if (isVoided) {
        footerLine2 = "VOID — This receipt has been cancelled and is not valid for tax deductions.";
    }

    if (rawTemplate) {
        const footerMatch = rawTemplate.match(/class=["']footer["'][^>]*>([\s\S]*?)<\/div>/i);
        if (footerMatch && footerMatch[1]) {
            const paragraphs = footerMatch[1].match(/<p[^>]*>([\s\S]*?)<\/p>/gi);
            if (paragraphs && paragraphs.length >= 1) {
                footerLine1 = stripHtml(paragraphs[0]).trim();
                if (paragraphs.length >= 2) {
                    footerLine2 = stripHtml(paragraphs[1]).trim();
                }
            }
        }
    }
